...
class TeacherGradingServiceTest {
    ...
    void getAttemptsToGrade_Success_WhenOwner_WithoutKeyword() {
        ...
        when(examAttemptRepository.findByExamIdAndStatus(eq(examId), eq(AttemptStatus.AWAITING_MANUAL_GRADING), eq(pageable)))
                .thenReturn(page);
        when(candidateAnswerRepository.countByExamAttemptIdAndGradingStatus(awaitingAttempt.getId(), GradingStatus.PENDING_MANUAL))
                .thenReturn(2L);
    ...
    void getAttemptsToGrade_Success_WhenOwner_WithSearchKeyword() {
        ...
        when(examRepository.findById(examId)).thenReturn(Optional.of(mathExam));
        when(examAttemptRepository.searchAttemptsByExamIdAndStatus(eq(examId), eq(AttemptStatus.AWAITING_MANUAL_GRADING), eq("%student%"), eq(pageable)))
                .thenReturn(page);
        when(candidateAnswerRepository.countByExamAttemptIdAndGradingStatus(awaitingAttempt.getId(), GradingStatus.PENDING_MANUAL))
                .thenReturn(2L);
    ...
    void getAttemptsToGrade_ThrowsAccessDenied_WhenNotOwner() {
        ...
        assertThrows(AccessDeniedException.class, () ->
                teacherGradingService.getAttemptsToGrade(
    ...
    void getAttemptDetailForGrading_Success() {
        ...
        AttemptGradingDetailResponse detail = teacherGradingService.getAttemptDetailForGrading(attemptId, teacherOwner);
    ...
    void submitEssayGrades_AllEssaysGraded_FinalizesToSubmitted() {
        ...
        GradeEssaySubmissionRequest request = GradeEssaySubmissionRequest.builder()
                ...
                .grades(List.of(
                        GradeEssayItemRequest.builder()
                                ...
                                .awardedScore(4.5)
                                .teacherFeedback("Great explanation, minor symbol typo.")
                                .build(),
                        GradeEssayItemRequest.builder()
        ...
        when(candidateAnswerRepository.countByExamAttemptIdAndGradingStatus(attemptId, GradingStatus.PENDING_MANUAL))
                .thenReturn(0L);
