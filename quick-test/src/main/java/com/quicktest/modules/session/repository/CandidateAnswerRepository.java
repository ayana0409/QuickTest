...
public interface CandidateAnswerRepository extends JpaRepository<CandidateAnswer, UUID> {
        ...
        /**
         * Eagerly fetch candidate answers along with Question details and selected
         * options.
         */
        ...
        /**
         * Paginated candidate answers for a specific question with attempt and user
         * eagerly fetched.
         */
        @Query(value = "SELECT ca FROM CandidateAnswer ca " +
                        "JOIN FETCH ca.examAttempt a " +
                        "LEFT JOIN FETCH a.user u " +
                        "WHERE ca.question.id = :questionId " +
                        "ORDER BY a.submitTime DESC NULLS LAST", countQuery = "SELECT COUNT(ca) FROM CandidateAnswer ca WHERE ca.question.id = :questionId")
        Page<CandidateAnswer> findByQuestionIdWithAttemptAndUser(
                        @Param("questionId") UUID questionId, Pageable pageable);

        /**
         * Paginated candidate answers for a specific question filtered by grading
         * status.
         */
        @Query(value = "SELECT ca FROM CandidateAnswer ca " +
                        "JOIN FETCH ca.examAttempt a " +
                        ...
                        "ORDER BY a.submitTime DESC NULLS LAST", countQuery = "SELECT COUNT(ca) FROM CandidateAnswer ca WHERE ca.question.id = :questionId AND ca.gradingStatus = :status")
        ...
        /**
         * Find all pending candidate answers for a specific question to feed into AI
         * batch grading.
         */
        ...
        /**
         * Find all pending essay answers across all questions in an exam for
         * entire-exam AI grading.
         */
        @Query("SELECT ca FROM CandidateAnswer ca " +
                        "JOIN FETCH ca.question q " +
                        "JOIN FETCH ca.examAttempt a " +
        ...
        /**
         * Bulk delete all candidate selected options for a specific question.
         */
        @Modifying(clearAutomatically = true)
        @Query(value = "DELETE FROM candidate_selected_options WHERE candidate_answer_id IN (SELECT id FROM candidate_answers WHERE question_id = :questionId)", nativeQuery = true)
        void deleteSelectedOptionsByQuestionId(@Param("questionId") UUID questionId);

        /**
         * Bulk delete all candidate answers for a specific question.
         */
        @Modifying(clearAutomatically = true)
        @Query("DELETE FROM CandidateAnswer ca WHERE ca.question.id = :questionId")
        void deleteByQuestionId(@Param("questionId") UUID questionId);

        /**
         * Batch count candidate answers grouped by questionId to avoid N+1 queries.
         */
        @Query("SELECT ca.question.id, COUNT(ca.id) FROM CandidateAnswer ca WHERE ca.question.id IN :questionIds GROUP BY ca.question.id")
        List<Object[]> countAttemptsByQuestionIds(@Param("questionIds") List<UUID> questionIds);
}
